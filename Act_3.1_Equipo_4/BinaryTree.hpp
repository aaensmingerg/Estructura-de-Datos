...
public:
    ...
    std::vector<T> printLeaf(std::vector<T>) const;
    std::vector<T> printLeaf(TreeNode<T> *, std::vector<T>) const;
...
template <class T>
std::vector<T> BinaryTree<T>::printLeaf(std::vector<T> hojas) const
{
    hojas = this->printLeaf( this->root, hojas);
    return hojas;
    
}

template <class T>
std::vector<T> BinaryTree<T>::printLeaf(TreeNode<T> * node, std::vector<T> hojas) const
{
    if (node != nullptr) {
        ...
        if (left == nullptr && right == nullptr ) {
            /* Procesar el nodo */
            std::cout << *node << " ";
            hojas.push_back(node->getInfo());
            return hojas;
        }
        else {
            /* Desplazarse a la izquierda */
            hojas = printLeaf( left,  hojas );
            
            /* Desplazarse a la derecha */
            hojas = printLeaf( right, hojas );
